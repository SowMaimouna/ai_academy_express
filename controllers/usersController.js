...
index: (req, res, next) => { 
  ...
  show: async (req, res, next) => {
    try {
      ...
      if (!user) {
        req.session.alertMessage = "Utilisateur non trouvé.";
        return res.redirect("/users");
      }
  
      // Rechercher les cours auxquels l'utilisateur est inscrit
      const enrollments = await Enrollment.find({ email: user.email }).populate("course");
